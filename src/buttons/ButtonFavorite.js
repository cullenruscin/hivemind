...
export const ButtonFavorite = ({ post, getPosts }) => {
        ...
        if (favorited) {
            fetch(`http://localhost:8088/favorites?postId=${post.id}&userId=${userId}`)
              ...
              .then(favorites => {
                ...
                fetch(`http://localhost:8088/favorites/${favoriteId}`, { 
                    method: 'DELETE' 
                })
                  .then(
                    () => {
    ...
    return (
        <button className="button is-light is-danger mr-1 is-small" onClick={(clickEvent) => handleFavoriteButtonClick(clickEvent)}>
            <span className="icon">
                {
                    favorited 
